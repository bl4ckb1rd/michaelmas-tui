        ...
        if tools:
            if isinstance(llm, ChatOllama):
                ...
                if check_ollama_tool_support(model_id):
                    logging.info(
                        f"AGENT ({agent_name}): Binding tools for capable Ollama model: {model_id}"
                    )
                    model = llm.bind_tools(tools)
                else:
                    logging.info(
                        f"AGENT ({agent_name}): Skipping tool binding for Ollama model: {model_id} (No {{ .Tools }} in template)"
                    )
                    model = llm
            else:
        ...
        for tool_call in last_message.tool_calls:
            ...
            if not tool_to_call:
                tool_messages.append(
                    ToolMessage(
                        content=f"Error: Tool {tool_name} not found or disabled.",
            ...
            try:
                logging.debug(
                    f"TOOL NODE: Calling {tool_name} with args: {tool_call['args']}"
                )
...
def create_agent_graph(
    ...
    if model_name.startswith("ollama:"):
        ...
        logging.info(
            f"Using ChatOllama with model: {ollama_model}, temperature: {temperature}, max_tokens: {max_tokens}"
        )
        llm = ChatOllama(
            model=ollama_model, temperature=temperature, num_predict=max_tokens
        )
    elif model_name.startswith("openai:"):
        openai_model = model_name.split(":", 1)[1]
        logging.info(
            f"Using ChatOpenAI with model: {openai_model}, temperature: {temperature}, max_tokens: {max_tokens}"
        )
        llm = ChatOpenAI(
        ...
        logging.info(
            f"Using ChatGoogleGenerativeAI with model: {model_name}, temperature: {temperature}, max_tokens: {max_tokens}"
        )
        llm = ChatGoogleGenerativeAI(
            model=model_name, temperature=temperature, max_output_tokens=max_tokens
        )
    ...
    workflow.add_conditional_edges(
        "supervisor",
        lambda x: x["next"],
        {"agent_sheets": "agent_sheets", "agent_general": "agent_general", "end": END},
    )
    ...
    # Sheets Loop Logic
    workflow.add_conditional_edges(
        "agent_sheets", should_continue, {"continue": "tool_sheets", "end": END}
    )
    workflow.add_edge(
        "tool_sheets", "agent_sheets"
    )  # Return to agent to digest tool output

    # General Loop Logic
    workflow.add_conditional_edges(
        "agent_general", should_continue, {"continue": "tool_general", "end": END}
    )
    workflow.add_edge(
        "tool_general", "agent_general"
    )  # Return to agent to digest tool output
...
async def run_agent(
    ...
    has_user_ai_messages = any(
        isinstance(msg, (HumanMessage, AIMessage)) for msg in history
    )
    ...
    async for event in app.astream_events(inputs, version="v2"):
        ...
        if kind == "on_chat_model_stream":
            ...
            if hasattr(output, "usage_metadata") and output.usage_metadata:
                ...
                and "token_usage" in output.response_metadata
            ):
