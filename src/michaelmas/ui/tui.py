...
class SettingsScreen(ModalScreen[dict]):
    ...
    def __init__(
        ...
        tools_enabled_master: bool = True,
    ):
        super().__init__()
    ...
    def compose(self) -> ComposeResult:
        max_tokens_val = (
            str(self.current_max_tokens) if self.current_max_tokens is not None else ""
        )
        ...
        yield Grid(
            ...
            Checkbox(
                "Enable Tool Use",
                value=self.tools_enabled_master,
                id="master-tool-check",
            ),
            Label("Select Tools:", id="tools-label"),
            SelectionList(
                *tool_selections,
                id="tools-list",
                disabled=not self.tools_enabled_master,
            ),
        ...
        if event.button.id == "save-btn":
            try:
                ...
                if not (0.0 <= temp_val <= 1.0):
                    self.app.notify(
                        "Temperature must be between 0.0 and 1.0", severity="error"
                    )
                    return
...
class TuiApp(App):
    ...
    async def on_mount(self) -> None:
        ...
        self.llm_max_tokens = (
            None  # Default max tokens (None means unlimited/model default)
        )
    ...
    def update_status_bar(self, last_cost: float):
        ...
        tokens_display = (
            f"MaxTok={self.llm_max_tokens}" if self.llm_max_tokens else "MaxTok=Auto"
        )
    ...
    def load_chat(self, conversation_id: str):
        ...
        if not data:
            self.write_to_log(
                f"[bold red]Error:[/ ] Could not load conversation {conversation_id}"
            )
            return
        ...
        self.write_to_log(
            f"[bold yellow]INFO:[/ ] Loaded conversation: {data.get('title')}"
        )
    ...
    def save_current_chat(self):
        ...
        self.current_conversation_id = storage.save_conversation(
            self.current_conversation_id, messages_data
        )
    ...
    async def run_agent_worker(self) -> None:
        ...
        logging.info(
            f"Running agent with model: {model}, temp: {temperature}, max_tok: {max_tokens}, tools: {len(enabled_tools)}"
        )
    ...
    def select_model_worker(self) -> None:
        ...
        all_models = (
            sorted(list(set(gemini_models)))
            + sorted(openai_models)
            + sorted(ollama_models)
        )

        self.call_from_thread(
            self.app.push_screen,
            ModelSelectionScreen(all_models),
    ...
    def list_models_worker(self) -> None:
        ...
        try:
            ...
            logging.warning(f"Failed to list Ollama models: {e}")
            models_info += (
                f"[dim]Could not connect to Ollama ({e}). Make sure it is running.[/]\n"
            )
    ...
    def on_model_selected(self, model_name: str) -> None:
        ...
        if model_name:
            ...
            self.write_to_log(
                f"[bold yellow]INFO:[/ ] Model set to: {self.current_model}"
            )
    ...
    def on_settings_changed(self, settings: dict) -> None:
        ...
        if settings:
            ...
            log_msg = (
                f"[bold yellow]INFO:[/ ] Settings updated: T={self.llm_temperature}"
            )
    ...
    async def on_chat_input_submitted(self, message: ChatInput.Submitted) -> None:
        ...
        if text.startswith("/"):
            ...
            if command == "/model":
                ...
                self.run_worker(
                    self.select_settings_worker, exclusive=True, thread=True
                )
            elif command == "/new":
