...
from textual.widgets import Input, TextArea, OptionList, Button, Checkbox, SelectionList
...
async def test_command_settings_screen():
    ...
    async with app.run_test() as pilot:
        ...
        pilot.app.run_worker(
            pilot.app.select_settings_worker, exclusive=True, thread=True
        )
        await pilot.pause()
...
async def test_run_agent_worker_flow():
    ...
    with patch("michaelmas.ui.tui.run_agent", side_effect=mock_run_agent):
        async with app.run_test() as pilot:
            ...
            # Run worker
            worker = pilot.app.run_worker(
                pilot.app.run_agent_worker, exclusive=True, thread=True
            )
            await worker.wait()
...
async def test_command_model():
    ...
    with (
        patch("michaelmas.ui.tui.OpenAI"),
        patch("michaelmas.ui.tui.ModelServiceClient"),
    ):
        async with app.run_test() as pilot:
            # Trigger model selection
            pilot.app.run_worker(
                pilot.app.select_model_worker, exclusive=True, thread=True
            )
...
async def test_load_chat_from_sidebar():
    ...
    async with app.run_test() as pilot:
        ...
        assert (
            pilot.app.current_conversation_id == "1"
        )  # From mock_storage.load_conversation
        log = pilot.app.query_one("#log", TextArea)
