...
def test_sheets_service_failure():
    with patch("michaelmas.tools.sheets.get_sheets_service", return_value=None):
        assert (
            create_spreadsheet.invoke("title") == "Failed to get Google Sheets service."
        )
        assert (
            get_sheet_data.invoke({"spreadsheet_id": "id", "range_name": "A1"})
            == "Failed to get Google Sheets service."
        )
        assert (
            update_sheet_data.invoke(
                {"spreadsheet_id": "id", "range_name": "A1", "values": []}
            )
            == "Failed to get Google Sheets service."
        )
        assert (
            append_to_sheet.invoke(
                {"spreadsheet_id": "id", "range_name": "A1", "values": []}
            )
            == "Failed to get Google Sheets service."
        )
