...
def test_get_sheets_service_token_exists(mocker):
    ...
    mock_build.assert_called_with("sheets", "v4", credentials=mock_creds.from_authorized_user_file.return_value)
    assert service == mock_build.return_value

def test_create_spreadsheet(mocker):
    mock_service = MagicMock()
    mocker.patch("michaelmas.tools.sheets.get_sheets_service", return_value=mock_service)
...
def test_get_sheet_data(mocker):
    ...
    result = sheets.get_sheet_data.invoke({"spreadsheet_id": "123", "range_name": "A1:B1"})
...
def test_update_sheet_data(mocker):
    ...
    result = sheets.update_sheet_data.invoke({"spreadsheet_id": "123", "range_name": "A1", "values": [["X"]]})
...
def test_append_to_sheet(mocker):
    ...
    result = sheets.append_to_sheet.invoke({"spreadsheet_id": "123", "range_name": "Sheet1", "values": [["X"]]})
